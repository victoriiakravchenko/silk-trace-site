...
const CATEGORY_META = CATEGORY_ORDER.map((label) => {
  const items = CATALOG.filter((p) => p.category === label);
  return { label, categoryKey: items[0] ? items[0].categoryKey : "other", count: items.length };
});
  ...
  return (
    <div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {["swim", "bra", "pajama", "socks_women", "accessory", "panty"].map((k, i) => {
            ...
            return (
              <button
                key={k + i}
                onClick={() => { setCategory(p.category); document.getElementById("catalog")?.scrollIntoView({ behavior: "smooth" }); }}
                className="lg-focus rounded-2xl aspect-square flex items-center justify-center p-5"
                style={{ background: swatchBackground(col), opacity: 0.92, marginTop: i % 2 ? 18 : 0 }}
              >
      ...
      <section className="max-w-6xl mx-auto px-5 pb-16">
        <p className="lg-eyebrow mb-4 text-center">Пройтись по категоріях</p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {CATEGORY_META.map((c) => (
            <button
              key={c.label}
              onClick={() => { setCategory(c.label); document.getElementById("catalog")?.scrollIntoView({ behavior: "smooth" }); }}
              className="lg-focus lg-card rounded-2xl p-4 flex items-center gap-3 text-left hover:shadow-sm transition-shadow"
              style={{ background: COLORS.latte }}
            >
              <div className="w-11 h-11 rounded-xl flex-shrink-0 flex items-center justify-center p-2" style={{ background: COLORS.aubergine }}>
                <CategoryArt category={c.categoryKey} stroke="#FFFFFF" />
              </div>
              <div className="min-w-0">
                <p className="text-[13px] leading-snug truncate" style={{ color: COLORS.ink }}>{c.label}</p>
                <p className="text-[11px] opacity-55" style={{ color: COLORS.ink }}>{c.count} моделей</p>
              </div>
            </button>
          ))}
        </div>
      ...
      {/* Catalog */}
      <section id="catalog" style={{ background: COLORS.latte }} className="pt-10 pb-4">
        <div className="max-w-6xl mx-auto px-5">
          <div className="flex items-center justify-between gap-4 flex-wrap mb-4">
            <h2 className="lg-display text-2xl" style={{ color: COLORS.ink }}>
              Каталог · {filtered.length} моделей
            </h2>
            <div className="flex gap-2 items-center">
              <span className="lg-eyebrow" style={{ color: COLORS.roseDark }}>Бренд:</span>
      ...
      <section style={{ background: COLORS.latte }} className="pb-16">
        <div className="max-w-6xl mx-auto px-5 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-2">
          ...
          {filtered.length === 0 && (
            <p style={{ color: COLORS.ink }} className="col-span-full text-center py-16 opacity-70">
        ...
        {visibleCount < filtered.length && (
          <div className="text-center mt-8">
            <button onClick={() => setVisibleCount((v) => v + 24)} className="lg-btn lg-focus px-6 py-2.5 rounded-full text-sm" style={{ border: "1px solid " + COLORS.aubergine, color: COLORS.aubergine }}>
